...
if __name__ == '__main__':
    ...
    if DATASET == 'INFIMNIST':
        ...
        train_loader = DataLoader(MyDataset(FEATURE_TEMPLATE%('train',0,10000), TARGET_TEMPLATE%('train',0,10000), transform=transform), batch_size=BATCH_SIZE, shuffle=True)
        test_loader = DataLoader(MyDataset(FEATURE_TEMPLATE%('test',0,10000), TARGET_TEMPLATE%('test',0,10000), transform=transform), batch_size=BATCH_SIZE, shuffle=True)
